...
public class AboutDateTimeAPI {
	...
	public void java8_datesAreImmutable() {
		...
		LocalDateTime date2 = date.plus(durationtoAdd); //  See if you can find a way to add the durationToAdd to the date
	...
	public void java8_timeInMiliSeconds() {
		...
		long timeInMilis = now.getEpochSecond(); // checkout if you can find the method on "now" that returns the milis
	...
	public void java8_getNow() {
		LocalDate now = LocalDate.now(); //  maybe there is a factory method on LocalDate you can use to get now
	...
	public void java8_dateInFuture() {
		...
		LocalDate future = now.plus(1, ChronoUnit.DAYS); // Find out what a ChronoUnit is and what it means.
	...
	public void java8_createSpecificDate() {
		LocalDate now = LocalDate.of(2000,11,23); // use the of method to create a date that represents 2000-11-23
	...
	public void java8_convertDateToDateTime() {
		...
		// A LocalDate does not store hours and minutes. See if you can enhance now with this information.
	...
	public void java8_getNowWithSpecificClock() {
		Clock clock = Clock.fixed(Instant.EPOCH,ZoneId.systemDefault()); // FIXME: create a clock that start at the epoch and uses the sytem timezone

		LocalDateTime now = LocalDateTime.now(clock).withHour(1).withMinute(0).withYear(1970).withMonth(1).withDayOfMonth(1);
	...
	public void java8_periodsAreUsedToDescribeDurationsInFullDays() {
		...
		Period period = Period.between(now, future); // calculate the period between now and future
	...
	public void java8_durationsAreUsedToDescribeDurationsMorePrecisely() {
		...
		Duration duration = Duration.between(now, future); // calculate the duration between now and future
	...
	public void java8_durationsCanAlsoBeUsedToCreateDate() {
		...
		LocalDateTime future = now.plus(duration); // add the duration to now
	...
	public void java8_adjustDatesBySemantic() {
		...
		LocalDate saturday = date.with(TemporalAdjusters.next(date.getDayOfWeek().plus(2))); // find out how to get the next Saturday.
	...
	public void java8_useFixedOffsetsForTimeZones() {
		// I wonder how many hours we need to pass in to have 28800 seconds
	...
	public void java8_useZoneIdsForTimeZones() {
		ZonedDateTime berlin = ZonedDateTime.now(ZoneId.of("Europe/Berlin")); // How do we find out the ZoneIf of Berlin?
