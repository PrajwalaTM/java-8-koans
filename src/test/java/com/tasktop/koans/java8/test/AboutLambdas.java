...
public class AboutLambdas {
	...
	public void java8_passingFunctionsWithLambdas_WithReturn_WithParentheses_WithCurlyBrackets() {
		...
		String result = yeller.yell("echo", (echo) -> {
			return echo; // Return the echo value
		});
	...
	public void java8_passingFunctionsWithLambdas_WithReturn_WithCurlyBrackets_AndMultiline() {
		...
		String result = yeller.yell("echo", echo -> {
			String myEcho = echo.concat(echoSufix); // Modify the echo and concatenate the echoSufix
			return myEcho;
		});
	...
	public void java8_passingFunctionsWithLambdas() {
		...
		//  Pass in a lambda like above but without any parentheses, brackets, return statement and semi colons
		String result = yeller.yell("echo", (Function<String, String>) echo -> echo);
	...
	public void java8_passingFunctionsWithLambdas_WithMethodReference() {
		...
		//  Method references are just another way to express a lambda. What we want here is is to call the
	...
	public void java8_passingSuppliers() {
		...
		String result = yeller.yell(() -> "echo"); // Let the supplier return the echo
	...
	public void java8_passingConsumers() {
		...
		yeller.yell("echo", echo -> {
			yeller.setEcho(echo);
		}); // consume the provided value and pass it to yeller.setEcho
	...
	public void java8_passingPredicates() {
		...
		boolean hasEcho = yeller.hasEcho(echo -> "echo".equals(echo)); // Fix the predicate to do a real equal check
